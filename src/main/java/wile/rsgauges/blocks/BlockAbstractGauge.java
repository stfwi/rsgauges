...
public abstract class BlockAbstractGauge extends RsDirectedBlock implements ModColors.ColorTintSupport
{
  ...
  public BlockAbstractGauge(long config, Block.Properties props, final AxisAlignedBB aabb, @Nullable ModResources.BlockSoundEvent powerOnSound, @Nullable ModResources.BlockSoundEvent powerOffSound)
  {
  ...
  public BlockAbstractGauge(long config, Block.Properties props, final AxisAlignedBB aabb)
  ...
  public static class TileEntityGauge extends RsBlock.RsTileEntity implements ITickableTileEntity
  {
    ...
    public void tick()
    {
      ...
      try {
        ...
        if(!(state.getBlock() instanceof BlockAbstractGauge)) return;
        final BlockAbstractGauge block = (BlockAbstractGauge) state.getBlock();
        if(world.isRemote) {
          ...
          } else if(block instanceof BlockGauge) {
            if((state.get(BlockGauge.POWER) != p)) world.setBlockState(pos, state.with(BlockGauge.POWER, p), 1|2|16); // |32
          }
        }
      } catch(Throwable e) {
        trigger_timer_ = 100;
