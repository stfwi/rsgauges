...
tasks["dist-check"] = function() {
  ...
  var fails = [];
  if(modversion=="") fails.push("Could not determine 'version_rsgauges' from gradle properties.");
...
tasks["sync-languages"] = function() {
  ...
  for(var name in output_data) {
    fs.writefile("./src/main/resources/assets/rsgauges/lang/" + name + ".lang", output_data[name]);
  }
};
...
tasks["version-check"] = function() {
  ...
  fs.readfile("gradle.properties", function(line){
    ...
    } else if(line.search(/^[\s]*version_rsgauges[\s]*=/i) >= 0) {
      modversion = line.replace(/^[^=]+=/,"").trim();
    }
    return false;
  })
  const combined_version = version_minecraft + "-" + modversion;
  var readme_version_found = fs.readfile("readme.md", function(line){
    ...
    return m[1]==modversion;
  });
  var ok=true;
  if(!readme_version_found) {
    alert("Version 'v" + modversion + "' not found in the readme changelog.");
    ok = false;
  }
  if(!ok) {
    alert("Version data:");
